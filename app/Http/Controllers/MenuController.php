...
class MenuController extends Controller
{
    ...
    {	
        if ($request->ajax()) {
            $data = Portal_Menu::select('id','nama_menu','no_urut','link','has_submenu')
                    ->orderBy('no_urut','asc')
                    // ->join('_portal_submenu', '_portal_submenu.id_menu', '=', '_portal_menu.id')
                    // ->join('_portal_sub_submenu', '_portal_sub_submenu.id_submenu', '=', '_portal_submenu.id')
                    ->get();
            ...
            return DataTables::of($data)
                    ...
                    ->editColumn('aksi', function($p){
                        return '
                            <a href="menu/'.$p->id.'/edit" class="btn btn-primary btn-sm"><i class="fas fa-fw fa-pen"></i></a>
                    ...
                    // })
                    ->addIndexColumn()
                    ->rawColumns(['id', 'has_submenu', 'aksi'])
                    ->make(true);
        }
    ...
    public function create()
    {
        ...
        return view('frontend.menu.create', compact('no_urut', 'year'));
    }
    ...
    {
        $data = Portal_Menu::where('id','=',$id)->get();

        return view('frontend.menu.edit', compact('data'));
    }
