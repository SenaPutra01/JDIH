...
<!-- Page Heading -->
<div class="d-sm-flex align-items-center justify-content-between @if(Auth::user()->instansiname != '') mb-4 @else mb-2 @endif">
    <h1 class="h3 mb-0 text-gray-800">Dashboard 
    
        @if(Auth::user()->instansiname == '')
        <span>
            <select name="finstansiname" id="finstansiname">
                <option value="">Semua Perangkat Daerah</option>
                @foreach($instansi as $in)
                <option value="{{ $in->name }}" @if($in->name == $ins) selected="selected" @endif>{{ $in->name }}</option>
                @endforeach
            </select>
        </span>
        @endif
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $auten }}</div>
                    </div>
                    <div class="col-auto">
...
$.get("{{ route('dashboard.getAreaChart', [':year', ':ins']) }}".replace(':year', $('#year').val()).replace(':ins', $('#finstansiname').val()), function(res){
...
$('#year, #finstansiname').change(function(){
    document.location.href = "{{ route('dashboard') }}?year=" + $('#year').val() + "&ins=" + $('#finstansiname').val();
});
...
$.get("{{ route('dashboard.getAreaPie', [':year', ':ins']) }}".replace(':year', $('#year').val()).replace(':ins', $('#finstansiname').val()), function(resp){
    lp = [];
    dp = [];
